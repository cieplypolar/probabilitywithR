...
summary(KM)
summary(KM, times = c(1,15*(1:10))) # I suppose bins for times

plot(KM, xlab="Days", main = 'Kaplan Meyer') 
autoplot(KM)

KM_02 <- survfit(Surv(time, status) ~ x, data=mydata) # depends on cycles
summary(KM_02, times = c(1,15*(1:10)))
print(KM_02)
...
surv_diff <- survdiff(Surv(time, status) ~ x, data = mydata) #log rank test
surv_diff

autoplot(survfit(Surv(time, status) ~ x, data=mydata))
